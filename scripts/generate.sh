...
for d in "${domains[@]}"; do
    ...
    command="python src/learning_to_plan/main.py --generate -c src/configs/generate/gemini.json -n ${number_of_problems} -d ${d} --task_type=${task_type} --prompt_type=${prompt_type} -s ${number_of_samples}"
