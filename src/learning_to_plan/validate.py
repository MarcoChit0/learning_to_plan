...
def validate_plans():
    ...
    config.log(f"Loaded {len(tasks)} tasks from {data_file_path}, of which {len(test_tasks)} are test tasks.", level=config.logging.INFO)
            ...
            for i, nlp_class in enumerate(t._model_generated_plans[model]):
                ...
                try:
                    ...
                    if is_plan_valid:
                        ...
                        config.log(f"VAL stdout:\n{result.stdout}", level=config.logging.DEBUG, do_print=False)
                        config.log(f"VAL stderr:\n{result.stderr}", level=config.logging.DEBUG, do_print=False)
                    tasks.add(t)
