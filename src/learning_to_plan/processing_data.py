...
#         is_plan_valid = False
#         if content.status != model.Model.Content.STATUS.ERROR:
#             try:        
#                 temp_plan_file = os.path.join(
#                 model.model_dir_path,
#                 f".temp_plan_{model.model_name}_{content.task.domain_file_path}_{content.task.instance_file_path}_{content.prompt_type}_{content.id}.txt".replace(" ", "_").replace("/", "_")
#                 )
#                 with open(temp_plan_file, "w") as f:
#                     f.write(content.pddl_plan)
#                 logger.debug(f"Created temporary plan file: {temp_plan_file}")
#                 cmd_list = [
#                     "utils/VAL/build/bin/Validate",
#                     "-v",
#                     "-t", "0.001",
#                     content.task.domain_file_path,
#                     content.task.instance_file_path,
#                     temp_plan_file
#                 ]
...
#                 logger.error(f"Error validating plan for task {content.task.id} - Model '{model.model_name}' - Prompt Type '{content.prompt_type}': {e}")
#                 raise e
...
#     **kwargs,
# ):
#     logger.info(f"Computing metrics for model {model.model_name} at {datetime.datetime.now()}.")
#     data = {}
#     for content in model.get_generated_plans():
#         if not content.was_validated():
#             print(f"skip -- {content.validity}")
#             continue 

#         if content.prompt_type not in data:
#             data[content.prompt_type] = {}

#         # convert dict into string to use as a key
#         prompt_metadata_key = str(content.prompt_metadata)
#         if prompt_metadata_key not in data[content.prompt_type]:
#             data[content.prompt_type][prompt_metadata_key] = {}
        
#         # convert dict into string to use as a key
#         model_metadata_key = str(content.model_metadata)
#         if model_metadata_key not in data[content.prompt_type][prompt_metadata_key]:
#             data[content.prompt_type][prompt_metadata_key][model_metadata_key] = {}
        
#         if content.task.domain not in data[content.prompt_type][prompt_metadata_key][model_metadata_key]:
#             data[content.prompt_type][prompt_metadata_key][model_metadata_key][content.task.domain] = {}
        
#         plan_size = "longer_plan" if content.task.is_longer_plan else "basic_plan"

#         if plan_size not in data[content.prompt_type][prompt_metadata_key][model_metadata_key][content.task.domain]:
#             data[content.prompt_type][prompt_metadata_key][model_metadata_key][content.task.domain][plan_size] = {}

#         if content.task.instance_file_path not in data[content.prompt_type][prompt_metadata_key][model_metadata_key][content.task.domain][plan_size]:
#             data[content.prompt_type][prompt_metadata_key][model_metadata_key][content.task.domain][plan_size][content.task.instance_file_path] = {
#                 'num_samples': 0,
#                 'num_valid_samples': 0,
#             }
        
#         data[content.prompt_type][prompt_metadata_key][model_metadata_key][content.task.domain][plan_size][content.task.instance_file_path]['num_samples'] += 1
#         if content.validity == model.Model.Content.VALIDITY.VALID:
#             data[content.prompt_type][prompt_metadata_key][model_metadata_key][content.task.domain][plan_size][content.task.instance_file_path]['num_valid_samples'] += 1
...
#     # Save results to CSV
#     csv_file_path = os.path.join(model.model_dir_path, f"metrics_{model.model_name}.csv".replace(" ", "_").replace("/", "_"))
#     results_df.to_csv(csv_file_path, index=False)
#     logger.info(f"Metrics for model {model.model_name} saved to {csv_file_path} at {datetime.datetime.now()}.")